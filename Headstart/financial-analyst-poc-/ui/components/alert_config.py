...
DEFAULT_WEBHOOK_URL = "http://localhost:5678/webhook-test/alert"


def get_webhook_url():
    """Return the configured n8n webhook URL, with env override support."""
...
def send_alert_config(email, ticker, threshold_type, threshold_value):
    ...
    try:
        ...
        return False, f"Webhook error {response.status_code}: {error_text}"

    except requests.exceptions.ConnectionError:
        return False, (
            f"Could not connect to the n8n webhook at {webhook_url}. "
            "Please make sure n8n is running and the alert workflow is active."
        )
