...
N8N_WEBHOOK_URL = "http://localhost:5678/webhook-test/alert"
...
def _coerce_to_dataframe(payload):
    """Normalize backend payloads into a pandas DataFrame without triggering bool ambiguity."""
    if payload is None:
        return pd.DataFrame()
    ...
    if isinstance(payload, dict):
        if not payload:
            return pd.DataFrame()
        if all(not isinstance(v, (dict, list, tuple, pd.Series, pd.DataFrame)) for v in payload.values()):
...
def get_stock_summary(ticker: str):
    ...
    try:
        ...
        return {
            ...
            "financials": {
                "income_statement": _coerce_to_dataframe(financial_statements.get('income_statement', {})),
                "balance_sheet": _coerce_to_dataframe(financial_statements.get('balance_sheet', {})),
                "cash_flow": _coerce_to_dataframe(financial_statements.get('cash_flow', {}))
            }
        }
    except Exception as e:
