...
from backend.utils.ticker_map import INDIA_TICKER_MAP, US_TICKER_MAP
...
COMMON_WORDS = {
    ...
    "NEWS", "LONG", "BEST", "TERM", "INDIA", "SHORT", "TICKER", "VALUE",
...
# EXPANDED: More finance keywords to catch broader queries
FINANCE_KEYWORDS = [
    ...
    "wealth", "retirement", "savings", "tax", "loan", "credit",
    "bank", "banking", "finance", "forex", "commodity",
    "gold", "silver", "oil", "futures", "options", "derivatives"
]
...
class FinancialAgent:
    ...
    def extract_tickers(self, query: str) -> list[str]:
        ...
        for token in tokens:
            ...
            # Top priority: known symbols do not depend on a live validation call.
            ...
            if token in US_TICKER_MAP:
                mapped = US_TICKER_MAP[token]
                found.add(mapped)
                logger.info(f"[Agent] Mapped US ticker: {token} → {mapped}")
                continue
    ...
    def classify_intent(self, query_lower: str, tickers: list[str]) -> str:
        ...
        if any(k in query_lower for k in ["price", "value", "cmp", "quote", "trading at"]) and tickers:
            return "price"
...
{str(data)[:600]}"""
    ...
    def run(self, query: str) -> dict:
        ...
        if intent == "financials" and primary:
            ...
            financials_data["key_stats"] = YFinanceHelper.get_key_stats(primary)
        ...
        # ====== KEY CHANGE: ALWAYS RESPOND ======
        # No "out_of_scope" blocking. Use Groq AI for ANY query.
        # This makes the chatbot accessible for finance AND general knowledge questions.
        
        steps.append({"thought": f"Using Groq AI to answer: {query}"})
