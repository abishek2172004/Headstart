...
from backend.utils.email_sender import send_alert_email
import logging
...
async def run_financial_graph(request: FinancialGraphRequest):
    ...
    try:
        ...
        if request.email and alert.get("alert_status") == "ALERT":
            subject = f"Stock alert: {request.ticker}"
            email_sent, email_message = send_alert_email(
                request.email,
                subject,
                alert.get("message", "A stock price alert was triggered."),
            )
